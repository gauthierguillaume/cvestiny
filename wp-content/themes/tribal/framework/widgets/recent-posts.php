...
class tribal_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'tribal_rp', // Base ID
			__('Tribal Recent Posts with Thumbnails', 'tribal'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'tribal' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'tribal' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'tribal' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','tribal' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'tribal' ); ?></label>
...
add_action( 'widgets_init', 'register_tribal_widget' );  
function register_tribal_widget() {  
    register_widget( 'tribal_Recent_Posts' );  
}  
